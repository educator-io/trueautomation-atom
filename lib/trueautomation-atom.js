...
export default {
  ...
  scanForTa(editor, range=null) {
    const projectName = this.getProjectName(editor);
    if (!range) {
      editor.scan(/[\s|\(|\=](ta|byTa|@FindByTA)\s*\((\s*|\s*taName\s*\=\s*)[\'\"](.+)[\'\"]\s*\)/g, {}, result => this.scanCallback(editor, result, projectName));
    } else {
      editor.scanInBufferRange(/[\s|\(|\=](ta|byTa|@FindByTA)\s*\((\s*|\s*taName\s*\=\s*)[\'\"](.+)[\'\"]\s*\)/g, range, result => this.scanCallback(editor, result, projectName));
    }
  },
