...
const TAExampleURL = 'https://trueautomation.io/docs/#/getting-started';
...
export default {
  ...
  taButton(taName, editor) {
    ...
    taButtonElement.addEventListener('click', async (event) => {
      ...
      const regexp = new RegExp(/^(?![:_])(?!.*(::))(?!.*(__))(?!.*(:_))(?!.*(_:))[a-z0-9:_]{1,2000}[^(:|_)]$/, 'ig');
      if (!regexp.test(taName)) {
        const err = `<b>Element name is not valid.</b>&nbsp;Only English letters, numbers, colon and underscore characters.
        Example: page:header:email_field <i><a href="https://trueautomation.io/docs/#/getting-started/human-readable-names">more info</a></i>`;
        return atom.notifications.addError(err, {dismissable: true});
      }
  ...
  findElements(projectName, result) {
    try {
      if (projectName) {
        const taName = result.match[3];
        const foundClass = async () => {
          ...
          if (elementsJson.status !== 200) {
            ...
            return elements.hasOwnProperty('elements') && elements.elements.length > 0 ? 'ta-found' : 'ta-not-found';
          }
        };
        return foundClass();
      }
  ...
  scanCallback(editor, result, projectName) {
    const callback = async () => {
      try {
        if (this.updateEditorText({ result, editor })) return null;
        const foundClass = this.findElements(projectName, result);
        return this.createTaMarkers(result, foundClass, editor);
      } catch (e) {
        console.log(e);
      }
    };
  ...
  scanForTa(editor, range=null) {
    const projectName = this.getProjectName(editor);
    if (!range) {
      editor.scan(/[\s|\(|\=](ta|byTa|@FindByTA)\s*\((\s*|\s*taName\s*\=\s*)[\'\"]((\w|:)+)[\'\"]\s*\)/g, {}, result => this.scanCallback(editor, result, projectName));
    } else {
      editor.scanInBufferRange(/[\s|\(|\=](ta|byTa|@FindByTA)\s*\((\s*|\s*taName\s*\=\s*)[\'\"]((\w|:)+)[\'\"]\s*\)/g, range, result => this.scanCallback(editor, result, projectName));
    }
  },
  scanForTaElement(editor, selector) {
    const projectName = this.getProjectName(editor);
    const regex = new RegExp("[\\s|\\(|\\=](ta|byTa|@FindByTA)\\s*\\((\\s*|\\s*taName\\s*\\=\\s*)[\\'\\\"](" + selector + ")[\\'\\\"]\\s*\\)", "g");
    editor.scan(regex, {}, result => this.scanCallback(editor, result, projectName));
  }
};
